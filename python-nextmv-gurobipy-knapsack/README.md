# Nextmv & gurobipy Python Integration - Knapsack

Example for running a Python application on the Nextmv Platform using the
`nextmv-gurobipy` package. We solve a knapsack Mixed Integer Programming problem.
...
2. Run the app.
   ...
   * With `uv`
      ...
      ```bash
      cat input.json | uv run main.py -TimeLimit 30 -MIPGap 0.01
      ```
