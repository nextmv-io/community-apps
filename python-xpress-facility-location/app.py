...
options = nextmv.Options(
    nextmv.Option(name="app_id", option_type=str, default="xpress-facility-location"),
    nextmv.Option(name="app_name", option_type=str, default="Xpress Facility Location"),
    nextmv.Option(name="action", option_type=str),
)

if options.action == "sync" or options.action == "push":
    # Initialize the cloud application.
    api_key = os.environ.get("NEXTMV_FICO_KEY")
    if api_key is None:
        raise Exception("Please set NEXTMV_FICO_KEY environment variable")
...
if options.action == "local":
    run_1 = local_app.new_run(input=nextmv.load())
    print(run_1)
