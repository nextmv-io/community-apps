...
class DecisionModel(nextmv.Model):
    def solve(self, input: nextmv.Input, duration: int) -> nextmv.Output:
        ...
        if result.is_feasible():
            ...
            for route in best.routes():
                ...
                vehicle_route, route_planned_ids = self._build_vehicle_route(
                    route=route,
                    vehicle=vehicle,
                    stops=stops,
    ...
    def _build_vehicle_route(
        ...
        for k, location_idx in enumerate(visits):
            ...
            prev_end_service = prev_sv.start_service + (
                0 if k == 0 else service_durations[visits[k - 1] - 2 * n_vehicles]
            )
