...
def solve(loaded_input, options) -> tuple[dict[str, Any], dict[str, Any]]:
    """Run the carwash simulation and return the solution and metrics."""
    simulation_events = []
    ...
    env.process(
        setup(
            ...
            washtime=data.get("WASHTIME"),
            t_inter=data.get("T_INTER"),
...
class Carwash:
    ...
    def wash(self, car, simulation_events):
...
def car(env, name, cw, simulation_events):
    ...
    with cw.machine.request() as request:
        ...
        yield env.process(cw.wash(name, simulation_events))
...
def setup(env, num_machines, washtime, t_inter, simulation_events):
    ...
    for _ in range(4):
        env.process(car(env, f"Car {next(car_count)}", carwash, simulation_events))
